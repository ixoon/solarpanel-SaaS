...
export async function generateMetadata({ params }: InstallerPageProps) {
  const { slug } = await params
  const installer = await getInstallerBySlug(slug)

  if (!installer) {
    return { title: "SolarApp — Solar Savings Calculator" }
  }

  return {
    title: `${installer.companyName} — Solar Savings Calculator`,
    description: `Check how much you could save with solar panels, powered by ${installer.companyName}.`,
  }
}
      ...
      <main className="flex flex-1 flex-col">
        ...
        <section className="flex flex-1 justify-center px-4 pb-16 sm:px-6 sm:pb-20">
          <div className="w-full max-w-lg -mt-8 sm:-mt-10">
            <CalculatorFlow installerSlug={installer.slug} />
          </div>
        </section>
      </main>
    </div>
